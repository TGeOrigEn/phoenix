...
/**
 * <p>Данный класс хранит контекст компонента.</p>
 * <ul>
 *     <p><b><i>Методы</i></b>:</p>
 *     <li>{@link #getTrace()} - возвращает список компонентов, порядок которых определяет вложенность текущего компонента;</li>
 *     <li>{@link #getParent()} - возвращает компонент, в контексте которого осуществляется поиск текущего компонента.</li>
 * </ul>
 */
